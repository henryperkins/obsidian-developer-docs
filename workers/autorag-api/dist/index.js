...
async function handleSearch(request, env, headers) {
  ...
  try {
    ...
    if (env.AI) {
      ...
      const result2 = useAiSearch ? await env.AI.autorag(env.AUTORAG_NAME).aiSearch(searchParams) : await env.AI.autorag(env.AUTORAG_NAME).search(searchParams);
    ...
    const apiResponse = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        ...body.rewrite_query !== void 0 && { rewrite_query: body.rewrite_query },
        ...body.model && { model: body.model },
...
async function handleIngestion(request, env, headers) {
  if (request.method !== "POST") {
    return new Response("Method not allowed", { status: 405, headers });
  }
...
async function handleRenderAndIngest(request, env, headers) {
  if (request.method !== "POST") {
    return new Response("Method not allowed", { status: 405, headers });
  }
  if (env.INGESTION_TOKEN) {
  ...
  const allowList = (env.ALLOWED_RENDER_HOSTS || "").split(",").map((s) => s.trim()).filter(Boolean);
  let target;
  try {
  ...
  try {
    const browser = await puppeteer_cloudflare_default.launch(env.MY_BROWSER);
    const page = await browser.newPage();
    await page.goto(target.href, { waitUntil: body.waitUntil || "networkidle", timeout: body.timeoutMs ?? 2e4 });
    ...
    await env.DOCS_BUCKET.put(key, html, {
      ...
      customMetadata: { source: "browser-render", url: target.href, timestamp: (/* @__PURE__ */ new Date()).toISOString() }
    });
...
__name(handleRenderAndIngest, "handleRenderAndIngest");
...
async function triggerAutoRAGSync(env) {
...
__name(triggerAutoRAGSync, "triggerAutoRAGSync");
export {
